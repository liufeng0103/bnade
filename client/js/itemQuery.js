...
var itemResource = {
	...
	search: function (url) {
		...
		$.ajax({
			url: url,
			async: false, // 同步
			crossDomain: !(document.all), // 解决IE9跨域访问问题
...
function loadChart(containerId, title, subtitle, chartLabels, chartMinBuyout, minBuyout, maxBuyout, showxAxisLabel, series1Type, chartQuantity, series2Type) {
	...
	$('#' + containerId).highcharts({
		...
		xAxis: [{
			categories: chartLabels, crosshair: true, labels: {
...
function loadHeatMapChart(container, title, heatMapLabels, heatMapData, minBuyout, seriesName, color) {
	$('#' + container).highcharts({
		...
		tooltip: {
			formatter: function () {
				return '<b>' + this.series.yAxis.categories[this.point.y] + '</b>的<b>' + this.series.xAxis.categories[this.point.x] + '</b><br>' + seriesName + '<b>' + this.point.value + '</b><br>';
			}
		},
		series: [{
			name: seriesName,
