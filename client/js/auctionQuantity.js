		...
		$.get("wow/auction/realms/summary", function(data) {
       		if (data.length === 0) {
       			...
       			for (var i in data) {
       				...
       				tblHtml += "<tr><td>"+(parseInt(i)+1)+"</td><td>"+realm+"</td><td>"+realmAuction.type+"</td><td>"+realmAuction.auctionQuantity+"</td><td>"+realmAuction.playerQuantity+"</td><td>"+realmAuction.itemQuantity+"</td><td>"+new Date(realmAuction.lastModified).format("MM-dd hh:mm:ss")+"</td></tr>";
       			}
       			tblHtml += "</tbody></table>";
